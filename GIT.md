...
1. **Modified**:
2. أنت عدلت الملف بس لسه ماقولتش للـ Git يسجله.
3. **Staged**: 
4. أنت علمت على الملف وقولت للـ Git "خد ده معاك في الـ Commit الجاية".
5. **Committed**: 
6. الداتا اتحفظت خلاص في قاعدة البيانات (الـ Repository) بتاعتك.
...
1. **`git init`**: 
2. بيعمل فولدر `.git` فاضي عشان يبدأ يخزن فيه الـ Objects.
3. **`git add file.txt`**:
4. هنا الـ Git بياخد محتوى الملف، وبيعمله ضغط (Hash) وبيحسب الـ **SHA-1** بتاعه (كود من 40 حرف)، وبيخزنه كـ **Blob Object** في الداتا بيز. وبعدين بيحدث الـ Index عشان يشاور على الـ Blob ده.
5. **`git commit`**: 
6. هنا الـ Git بيعمل حاجتين:
...
---
تمام، جهز قهوتك عشان هندخل في منطقة "العالمية" 🌍.

إحنا لحد دلوقتي كنا بنلعب في "الصندوق الرمل" (Sandbox) بتاعك على جهازك. دلوقتي جه الوقت نفهم إزاي Git بيخليك تشتغل مع ناس تانية في كوكب تاني. الموضوع ده اسمه **Distributed Git** (Git الموزع).

زي ما طلبت، هنبدأ بـ "الدسم النظري" عشان تفهم الميكانيزم اللي Git شغال بيه "Under the hood" لما بيكلم سيرفرات، وبعدين نخش على الأوامر.

---

# الفصل الرابع: العمل الموزع (Distributed Git & Remotes)

### 🔑 Key Takeaways

- في Git، كل جهاز هو "سيرفر" محتمل. النسخة اللي عندك (Clone) هي نسخة طبق الأصل من السيرفر، مش مجرد ملفات شغل.
- الـ **Remote Tracking Branches** (زي `origin/master`) هي "مرايات" بتعكس حالة السيرفر آخر مرة كلمته فيها، وأنت مبتعرفش تعدل عليها بايدك.
- الـ `pull` هي خدعة؛ هي عبارة عن `fetch` وراه `merge`.

## 1. النظرية: يعني إيه Distributed؟ (Distributed vs Centralized)

في الأنظمة القديمة (Centralized) زي SVN، كان فيه سيرفر واحد كبير هو "الإله"، وأنت مجرد "عميل" (Client) بتاخد ملف، تعدله، وترجعه. لو السيرفر وقع، الكل عطل.

في **Git (Distributed)**، الموضوع مختلف تماماً:

1. لما بتعمل `git clone`، أنت مش بس بتنزل أحدث ملفات.. لا، أنت بتنسخ **قاعدة البيانات بالكامل** (كل التاريخ، كل الـ Commits، كل الـ Branches) عندك.
2. ده معناه إن جهازك بقى عبارة عن "Full Backup" للسيرفر. لو السيرفر اتحرق، ممكن نستخدم جهازك عشان نبني سيرفر جديد.
3. بما إن كل واحد عنده نسخة كاملة، ده بيسمح بـ Workflows معقدة جداً، زي إنك تاخد شغل من زميلك مباشرة من غير ما تعدوا على السيرفر الرئيسي (Peer-to-Peer).

## 2. ميكانيكا الـ Remotes (تحت الكبوت)

الـ **Remote** في Git هو مجرد اسم دلع (Alias) لرابط السيرفر (URL). الاسم الافتراضي هو `origin`، وده الاسم اللي Git بيسميه للسيرفر اللي أنت عملت منه `clone`..

### الـ Remote Tracking Branches (أهم مفهوم هنا)

لما بتكلم السيرفر، Git بيخزن حالة السيرفر دي فين؟ بيخزنها في نوع خاص من الفروع اسمه **Remote-tracking branches**. شكلها بيبقى كده: `<remote>/<branch>` (مثلاً `origin/master`).

> [!NOTE] الفرق الجوهري
> 
> - **Local Branch (`master`):** ده الفرع بتاعك، بتعمل عليه commit ويتحرك معاك.
> - **Remote-tracking Branch (`origin/master`):** ده "مؤشر" (Pointer) بيقولك: "الماستر على السيرفر كان واقف فين آخر مرة كلمناه". المؤشر ده **Read-only** بالنسبة ليك، مبتحركوش بايدك، هو بيتحرك بس لما تعمل `fetch` أو `push`.

### 🕸️ المخطط البصري (Synchronization State)

الرسمة دي هتوضحلك إزاي Git بيشوف الفرق بين شغلك وشغل السيرفر:

```mermaid
graph TD
    subgraph Server_Repo ["GitHub Server"]
        S1["Commit A"] --> S2["Commit B"]
        S2 --> S3["Commit C"]
        ServerMaster["master branch"] --> S3
    end

    subgraph Local_Repo ["Your Machine"]
        L1["Commit A"] --> L2["Commit B"]

        %% Remote Tracking Branch points to where server was last time we checked
        OriginMaster["origin/master <br/> (Remote Tracking)"] --> L2

        L2 --> L3["Commit D <br/> (Your New Work)"]
        LocalMaster["master <br/> (Local Branch)"] --> L3
    end

    style ServerMaster fill:#f96,stroke:#333
    style OriginMaster fill:#ccc,stroke:#333,stroke-dasharray: 5 5
    style LocalMaster fill:#6f9,stroke:#333
```

**في الرسمة دي:**

1. السيرفر سابقك بـ Commit C.
2. أنت عندك شغل جديد (Commit D).
3. الـ `origin/master` عندك لسه واقف عند B (لأنك لسه معملتش Fetch، فمتعرفش إن السيرفر وصل لـ C).

---

## 3. الأوامر (Commands)

دلوقتي نفهم الأوامر بناءً على النظرية دي.

### `git remote` (إدارة السيرفرات)

ده الأمر اللي بتدير بيه اتصالاتك.

```bash
# اعرض السيرفرات المتسجلة
git remote -v

# ضيف سيرفر جديد (مثلاً Fork لزميلك) وسميه 'pb'
git remote add pb https://github.com/paulboone/ticgit

# معلومات تفصيلية عن السيرفر (بيوريك الفروع وحالتها)
git remote show origin
```

### `git fetch` (التحديث الآمن)

ده أهم أمر للفهم. `git fetch` بيروح للسيرفر، وينزل أي داتا جديدة (Commits/Objects) مش عندك، ويحدث الـ Remote-tracking branches (`origin/master`). **مهم جداً:** الأمر ده **مبيغيرش ملفاتك** (Working Directory) ولا بيعمل Merge. هو بس بيحدث قاعدة البيانات والمؤشرات المخفية.

```bash
# هات الجديد من السيرفر بس متلمسش شغلي
git fetch origin
```

### `git pull` (الأمر السحري)

المبتدئين بيحبوه، والمحترفين بيحذروا منه. `git pull` = `git fetch` + `git merge`. يعني: "روح هات الجديد من السيرفر، وادمج لي (Merge) الفرع اللي هناك (`origin/master`) في الفرع اللي أنا واقف عليه حالياً".

```bash
# هات الجديد وادمجه في شغلي حالاً
git pull
```

> [!WARNING] تحذير لو في شغل متضارب (Conflicts)، الـ `git pull` هيدخلك في خناقة الـ Merge فوراً. عشان كده الأفضل تعمل `fetch` الأول وتشوف الوضع، وبعدين تعمل `merge` بمزاجك.

### `git push` (النشر)

عشان ترفع شغلك للسيرفر. الأمر ده بياخد الـ Commits اللي عندك ومش موجودة فوق، يرفعها، وبعدين يحرك مؤشر السيرفر عشان يطابق مؤشرك.

```bash
# ارفع الفرع master للسيرفر origin
git push origin master
```

### `git branch -u` (الربط - Tracking)

عشان تربط فرع محلي بفرع على السيرفر (عشان لما تكتب `git pull` بس، هو يفهم يسحب منين). دي بتتعمل أوتوماتيك لما تعمل Clone، بس ممكن تعملها بايدك:

```bash
# اربط الفرع الحالي بـ origin/master
...
## 4. سيناريو عملي (Workflow Code Snippet) 🚀

تعالى نطبق سيناريو إنك بتشتغل مع تيم، وفيه شغل حصل على السيرفر وأنت شغال:

```bash
# 1. شوف السيرفرات اللي عندك
git remote -v

# 2. هات أحدث حالة من السيرفر (من غير دمج)
git fetch origin

# 3. قارن بين شغلك (master) وبين اللي جالك من السيرفر (origin/master)
# ده هيوريك ايه اللي زمايلك عملوه وأنت لسه مدمجتوش
git log master..origin/master

# 4. لو الدنيا تمام، ادمج الشغل في فرعك
git merge origin/master

# 5. دلوقتي أنت محدث (Synced)، اعمل شغلك
...
**ملخص سريع:** الـ Git الموزع بيعتمد على إنك بتحدث "صورة" السيرفر عندك (`fetch`)، وبعدين تدمجها في شغلك (`merge`)، وبعدين ترفع النتيجة (`push`).

الخطوة الجاية؟ ممكن نتكلم عن **GitHub** كمنصة (Pull Requests & Forking) لأن ده التطبيق العملي الأشهر للكلام ده، أو ندخل في **Git Tools** المتقدمة زي `stashing` و `rewriting history`. تحب تكمل في إيه؟